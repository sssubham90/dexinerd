...
function FAQ() {
	...
	return (
		<div
			id="faqs"
			className="scroll-mt-30 my-27 mx-40 flex flex-row justify-between items-center bg-[#F8F8F8] max-[1024px]:mx-4"
		>
			...
			<div className="w-253 max-[1520px]:w-full">
				...
				>
					{FAQs.map((faq, index) => (
						// biome-ignore lint/suspicious/noArrayIndexKey: FAQs are static and won't change, so using index as key is acceptable here
						<Item key={index} value={`item-${index + 1}`}>
							<Trigger className="py-2.5 border-b data-[state=open]:border-[#DD5D18] data-[state=open]:text-[#DD5D18] data-[state=closed]:border-[#66666633] data-[state=close]:text-[#666666] w-full flex justify-between items-center cursor-pointer">
